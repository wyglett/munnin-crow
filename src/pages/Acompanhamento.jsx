...
import { Plus, Activity, Calendar, DollarSign, Loader2, ArrowRight } from "lucide-react";
...
export default function Acompanhamento() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["acompanhamentos"] }); setDialogOpen(false); },
  });
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Acompanhamento de Projetos</h1>
            <p className="text-gray-500 text-sm mt-1">Gerencie projetos contratados — gastos, relatórios e documentação</p>
          </div>
        ...
        {isLoading ? (
          ...
          <Card>
            <CardContent className="text-center py-16">
              ...
              <p className="text-gray-500 font-medium">Nenhum projeto em acompanhamento</p>
              ...
              <Card key={p.id} className="border-l-4 border-l-indigo-600 hover:shadow-md transition-shadow">
                <CardContent className="p-5 flex items-center justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    {p.descricao_projeto && <p className="text-sm text-gray-600 line-clamp-1 mt-0.5">{p.descricao_projeto}</p>}
                  ...
                  <Link to={createPageUrl(`ProjetoDetalhe?id=${p.id}`)}>
                    <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700 flex-shrink-0">Gerenciar <ArrowRight className="w-3.5 h-3.5 ml-1" /></Button>
                  </Link>
                </CardContent>
              </Card>
            ))}
