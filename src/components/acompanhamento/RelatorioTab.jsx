...
import {
  ...
  CheckCircle2, Unlock, Sparkles, Info, RefreshCw, BookOpen
} from "lucide-react";
    ...
    <div className={`border rounded-xl overflow-hidden transition-all ${campo.concluido ? "border-green-200 bg-green-50/30" : "border-gray-200 bg-white"}`}>
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          {campo.concluido && campo.resposta && (
...
function TabelaExecucaoFinanceira({ gastos, orcamentoLinhas, campo, onChange }) {
  ...
  return (
    <div className="border rounded-xl overflow-hidden bg-white">
      <div
        className="flex items-start gap-3 p-4 cursor-pointer hover:bg-gray-50/50"
      ...
      {aberto && !campo.concluido && (
        <div className="px-4 pb-4 border-t border-gray-100 pt-3 space-y-3">
          <div className="flex justify-between items-center">
            <p className="text-xs text-gray-500">{gastos.length} item(s) no financeiro</p>
...
function ImportProjetoAprovado({ projeto, onSave }) {
  ...
  const extrairDoProjeto = async () => {
    ...
    setExtraindo(true);
    const r = await base44.integrations.Core.InvokeLLM({
      prompt: `Analise este documento de projeto aprovado. Extraia as seguintes informações estruturadas:
1. Atividades previstas (lista)
2. Cronograma de execução (fases/meses)
3. Equipe do projeto (nomes, funções)
4. Linhas de gasto/orçamento (categoria, descrição, valor aprovado)
5. Objetivos e metas
Retorne tudo de forma estruturada.`,
      file_urls: [fileUrl],
  ...
  const aplicarNoOrcamento = async () => {
    ...
    const linhasExistentes = projeto.orcamento_linhas || [];
    await onSave({ orcamento_linhas: [...linhasExistentes, ...novasLinhas] });
    setOpen(false);
    setResultado(null);
  };
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-purple-600" /> Importar do Projeto Aprovado
            </DialogTitle>
          </DialogHeader>
                ...
                {resultado.atividades?.length > 0 && (
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <ul className="text-sm text-gray-700 space-y-0.5">
                      {resultado.atividades.slice(0, 5).map((a, i) => <li key={i} className="text-xs">• {a}</li>)}
                      {resultado.atividades.length > 5 && <li className="text-xs text-gray-400">...e mais {resultado.atividades.length - 5}</li>}
                    </ul>
                  </div>
                )}
                {resultado.equipe?.length > 0 && (
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {resultado.equipe.map((m, i) => <Badge key={i} variant="outline" className="text-xs">{m.nome} — {m.funcao}</Badge>)}
                    </div>
                  </div>
                )}
                {resultado.linhas_orcamento?.length > 0 && (
                  <div className="bg-indigo-50 rounded-lg p-3">
                    ...
                    <div className="space-y-1">
                      {resultado.linhas_orcamento.map((l, i) => (
                        <div key={i} className="flex justify-between text-xs text-gray-700">
                          <span>{l.categoria} — {l.descricao}</span>
                          <span className="font-bold">{fmt(l.valor_aprovado)}</span>
                        </div>
                      ))}
                    </div>
                    <Button size="sm" onClick={aplicarNoOrcamento} className="mt-3 bg-indigo-600 hover:bg-indigo-700 w-full">
                      Aplicar no Orçamento do Projeto
                    </Button>
                  </div>
                )}
              </div>
...
Para cada campo, retorne: secao (título da seção), pergunta (o que precisa ser preenchido), tipo_resposta ("texto_longo", "texto_curto", "numero", "data", ou "tabela_itens" se for tabela de itens com descrição, quantidade, valor).
