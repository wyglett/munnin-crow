...
const BLOCK_TYPES = [
  ...
  { value: "secao", label: "Seção/Área", icon: "§" },
...
function BlocoPreview({ bloco, onEdit, onDelete }) {
  ...
  const getPreview = () => {
    switch (bloco.type) {
      ...
      case "secao": return <div className="text-sm font-bold text-indigo-700 bg-indigo-50 px-3 py-2 rounded border-l-4 border-indigo-500">{bloco.label || "[Seção]"}</div>;
        ...
        <div className="space-y-4">
          ...
          {form.type !== "pagina_quebra" && (
            <div>
              ...
              <Input value={form.label || ""} onChange={(e) => setForm({ ...form, label: e.target.value })} placeholder="Ex: Área Principal, Nome do Campo..." className="mt-1" />
            </div>
          )}
          ...
          {(form.type.startsWith("campo_") || form.type === "tabela" || form.type === "secao") && campos?.length > 0 && (
            <div>
              <label className="text-xs font-bold text-gray-600 uppercase">Vincular Campo/Área (opcional)</label>
              <Select value={form.campo_id || ""} onValueChange={(v) => setForm({ ...form, campo_id: v })}>
                <SelectTrigger className="mt-1"><SelectValue placeholder="Nenhum" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value={null}>Nenhum</SelectItem>
                  {campos.map(c => <SelectItem key={c.id} value={c.id}>{c.area}{c.subarea ? ` > ${c.subarea}` : ""}: {c.pergunta}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          )}
...
export default function ModeloTemplateEditorProposta({ open, onClose, modelo, onSave }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Editor de Template - {modelo?.nome}</DialogTitle>
        </DialogHeader>

        {!previewMode ? (
          <div className="space-y-4">
            <p className="text-xs text-gray-600">
              Monte o layout do documento de proposta. Cada bloco será preenchido com os dados quando a proposta for gerada/exportada.
            </p>
              ...
              {blocos.map((bloco) => {
                const getPreview = () => {
                  switch (bloco.type) {
                    ...
                    case "secao": return <div className="text-sm font-bold text-indigo-700 bg-indigo-50 px-3 py-2 rounded border-l-4 border-indigo-500">{bloco.label}</div>;
